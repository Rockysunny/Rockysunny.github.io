d3.csv("animaldata.csv",function(error,chartdata){
	...
	var chartHeight = maxCount*9;
	var width = (maxYear-minYear)*12
	var offset = 0;
	var count = 0;
	var columnWidth = 12;
	var DataObject = [];
	...
	function Chartpos(sectionFilter){
		...
		function BetterArray (dataArray){
			...
			for (var year in chartdataYearNest){
				var yearData = chartdataYearNest[year].key;
				var xPos = 12*(chartdataYearNest[year].key - minYear)+offset;
				var yPos;
				var id;
				var nestCount;
